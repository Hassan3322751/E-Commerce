        ...
        {foods &&
          foods.map(food => (
            <div key={food.id} className={classes.list_item}>
              ...
              <Link to={'/food/' + food.id} style={{color: 'dimgray'}}>{food.name}</Link>
              <Price price={food.price} />
              <div className={classes.actions}>
                <Link to={'/admin/editFood/' + food.id} className={classes.edit}>Edit</Link>
                <Link onClick={() => deleteFood(food)} className={classes.delete}>Delete</Link>
              </div>
            </div>
          ))}
      </div>
